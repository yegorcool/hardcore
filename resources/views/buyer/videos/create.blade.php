        ...
        <div class="col-lg-9">
            <form method="POST" action="{{ route('buyer.videos.store') }}" enctype="multipart/form-data">
                ...
                <div class="p-4">
                    ...
                    {{--File Upload--}}
                    <div>
                        <x-input-label for="video_create" ><span class=" block mb-2">{{__('Видео: ')}} <span class="text-base  font-normal text-gray-400">видео в формате .mp4</span></span>
                            <div class="preview-btn ml-2">{{__('Выберите файл')}}</div>
                        </x-input-label>
                        <x-text-input id="video_create" class="hidden" type="file" name="video_file" :value="old('video_file')"/>
                    ...
                    <!-- comment -->
                    <div class="lg:w-2/3 mt-2">
                        <x-input-label for="comment" :value="__('Ваше пожелание бойцу ')"/>
