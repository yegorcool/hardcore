...
@section('titlebutton')
    <div class="theme-btn ">
        <a class="text-white hover:text-gray-100" href="" class=""><i class="fa fa-plus mr-2"></i>
            {{ __(' Добавить бойца') }}
        </a>
    </div>
@endsection
...
@section('content')
    <section class="container-fluid  px-lg-5 mx-auto sm:px-6 lg:px-8">
        <div class="w-full ">
            ...
            <div class="my-4">
                <table
                    class="table table-responsive table-dark table-sm table-hover bg-black align-middle text-gray-200">
                    ...
                    @forelse($fighters as $fighter )
                        <tr>
                            ...
                            <td><a href="#" class="hover:text-themeOrange">{{$fighter->name}}</a></td>
                            <td>{{$fighter->email}}</td>
                            <td>{{$fighter->city}}</td>
                            <td class="text-right">{{$fighter->height}}</td>
                            <td>{{$fighter->weight}}</td>
                            <td>{{$fighter->description}}</td>
                            <td><a href="#" class="theme-btn bg-white/50 mx-2 text-sm">Редактировать</a></td>
